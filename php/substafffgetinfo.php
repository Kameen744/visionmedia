...
if (isset($_SESSION['vsubuserp'])) {
	...
	$monqur = $vfms_con->query("SELECT `Staff_ID`, `C_Name`, `C_Number`, `C_Email`, 
	`C_State`, `C_Lga`, `C_Address`, `C_Dateofbirth`, `C_Maritalstatus`, `C_Gender`, 
	`C_Qualification`, `C_Nationality` FROM `vsn_staff` WHERE Station_ID = '$stID' 
	AND Staff_ID = '$StfID' ");
	
	$prow = $monqur->fetch_array(MYSQLI_NUM);

	if (!empty($prow[0])) {
	$staffID = ucfirst($prow[0]); $staffName = ucfirst($prow[1]); $staffNumber = ucfirst($prow[2]);
	$staffEmail = ucfirst($prow[3]); $staffState = ucfirst($prow[4]); $stafflga = ucfirst($prow[5]);
	$staffaddress = ucfirst($prow[6]); $staffdateobirth = ucfirst($prow[7]); $staffmaritalstatus = ucfirst($prow[8]);
	$staffGender = ucfirst($prow[9]); $staffqualification = ucfirst($prow[10]); $staffnationality = ucfirst($prow[11]);
	...
	$sprm = ucfirst($scrow[0]); $sprmql = ucfirst($scrow[1]); $sprmfrm = ucfirst($scrow[2]); $sprmto = ucfirst($scrow[3]);
	$ssec = ucfirst($scrow[4]); $ssecql = ucfirst($scrow[5]); $ssecfrm = ucfirst($scrow[6]); $ssecto = ucfirst($scrow[7]);
	$ster = ucfirst($scrow[8]); $sterql = ucfirst($scrow[9]); $sterfrm = ucfirst($scrow[10]); $sterto = ucfirst($scrow[11]);
	...
	<div id='cv' class='instaFade'>
		<div class='mainDetails'>
			...
			<div id='contactDetails' class='quickFade delayFour'>
				<ul>
					<li>E-mail: <a href='#' target='_blank'>$staffEmail</a></li>
...
<div id='mainArea' class='quickFade delayFive'>
	<section>
		<article>
			<div class='sectionTitle' style='width:300px; '>
...
// sch 1
	...
	$sne = ucfirst($scrow[12]); $sneql = ucfirst($scrow[13]); 
	$snefrm = ucfirst($scrow[14]); $sneto = ucfirst($scrow[15]);
...
//sch 2
	...
	$stw = ucfirst($scrow[16]); $stwql = ucfirst($scrow[17]); 
	$stwfrm = ucfirst($scrow[18]); $stwto = ucfirst($scrow[19]);
...
//sch 3
	...
	$str = ucfirst($scrow[20]); $strql = ucfirst($scrow[21]); 
	$strfrm = ucfirst($scrow[22]); $strto = ucfirst($scrow[23]);
	$table .= "
	<section></section>
		<div>
...
//sch 4
	...
	$sfr = ucfirst($scrow[24]); $sfrql = ucfirst($scrow[25]); 
	$sfrfrm = ucfirst($scrow[26]); $sfrto = ucfirst($scrow[27]);
	$table .= "
	<section></section>
		<div>
...
// Start Working Experience
	...
	$job_1 = ucfirst($exrow[10]); $jobCom_1 = ucfirst($exrow[11]); $joFrm_1 = ucfirst($exrow[12]); $jobto_1 = ucfirst($exrow[13]);
	$table .= "
	...
	$job_2 = ucfirst($exrow[14]); $jobCom_2 = ucfirst($exrow[15]); $joFrm_2 = ucfirst($exrow[16]); $jobto_2 = ucfirst($exrow[17]);
	$table .= "
	<section></section>
		<div>
	...
	$job_3 = ucfirst($exrow[18]); $jobCom_3 = ucfirst($exrow[19]); $joFrm_3 = ucfirst($exrow[20]); $jobto_3 = ucfirst($exrow[21]);
	$table .= "
	<section></section>
		<div>
...
//start Skills
	...
	$skil_1 = ucfirst($exrow[0]); $skilDes_1 = ucfirst($exrow[1]);
	$table .= "
		<div>
			<h3 style='width:450px; float:left; font-weight:bold;'>$skil_1</h3>
			<h3 style=''>$skilDes_1</h3>
		<div>
	";
	} else {
	...
	if (!empty($exrow[2]) & !empty($exrow[3])) {
		$skil_2 = ucfirst($exrow[2]); $skilDes_2 = ucfirst($exrow[3]);
	$table .= "
	<section></section>
		<div>
			<h3 style='width:450px; float:left; font-weight:bold;'>$skil_2</h3>
			<h3 style=''>$skilDes_2</h3>
		<div>
	";
	} else {
	...
	if (!empty($exrow[4]) & !empty($exrow[5])) {
		$skil_3 = ucfirst($exrow[4]); $skilDes_3 = ucfirst($exrow[5]);
	$table .= "
	<section></section>
		<div>
			<h3 style='width:450px; float:left; font-weight:bold;'>$skil_3</h3>
			<h3 style=''>$skilDes_3</h3>
		<div>
	";
	} else {
	...
	<section></section>
		<div>
			<h3 style='width:450px; float:left; font-weight:bold;'>$skil_4</h3>
			<h3 style=''>$skilDes_4</h3>
		<div>
	";
	} else {
...
// Start Referee
	...
	if (!empty($rfrow[0]) & !empty($rfrow[1]) & !empty($rfrow[2]) & !empty($rfrow[3])) {
		$ref_1 = ucfirst($rfrow[0]); $rfttl_1 = ucfirst($rfrow[1]); $rfnum_1 = ucfirst($rfrow[2]); $rfadd_1 = ucfirst($rfrow[3]);
	$table .= "
		<div>
	...
	$ref_2 = ucfirst($rfrow[4]); $rfttl_2 = ucfirst($rfrow[5]); $rfnum_2 = ucfirst($rfrow[6]); $rfadd_2 = ucfirst($rfrow[7]);
	$table .= "
	<section></section>
		<div>
	...
	$ref_3 = ucfirst($rfrow[8]); $rfttl_3 = ucfirst($rfrow[9]); $rfnum_3 = ucfirst($rfrow[10]); $rfadd_3 = ucfirst($rfrow[11]);
	$table .= "
	<section></section>
		<div>
